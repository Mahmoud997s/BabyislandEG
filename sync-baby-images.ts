...
async function scrapeImage(url: string) {
    try {
        ...
        for (const pattern of patterns) {
            ...
            if (match && match[1]) {
                // If dynamic image JSON, parsing it might be needed, but let's see what we get
                let imgUrl = match[1];
                if (imgUrl.startsWith('{')) {
                    // It's the dynamic image JSON object keys
                    // data-a-dynamic-image="{"https://...": [x,y], ...}"
                    // We need the first key
                    try {
                        const json = JSON.parse(imgUrl.replace(/&quot;/g, '"'));
                        imgUrl = Object.keys(json)[0];
                    } catch (e) {
                         // simple regex extract from the string if parse fails
...
async function run() {
    console.log('Fetching baby-care products with source_url...');
    console.log('Fetching baby-care products with source_url...');
