...
export function isValidCategory(ids: any): boolean {
    return Array.isArray(ids) && ids.length >= 2 && VALID_CATEGORIES.has(ids[1]);
}

export function resolveCategory(ids: any): { ids: string[], legacy?: string, needsReview: boolean } {
...
export function mapDbToProduct(dbItem: any): Product {
    ...
    const processedImages = rawImages.map((img: string) => {
        ...
        // Use environment variable if available, otherwise relative path or placeholder
        const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL || 'https://placeholder.supabase.co';
        return `${supabaseUrl}/storage/v1/object/public/store-assets/${img}`;
    });
